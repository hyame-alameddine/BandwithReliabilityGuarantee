...
public class VirtualMachine extends Node{
	...
	//this is the backup VM backing Up this one (set for primary VMs)
	public VirtualMachine backupVm;
	
	//list of vms backed up by this (not populated yet)
	...
	/**
	 * This function clones all the elements of the VM and points to a clone objects ( pm)
	 * Note : the request and the backup VM are set later after creating them 
	 * 
	 * @param pm physical machine hosting the VM. if null this.pm is cloned
	 * 
	 * @return cloned VM
	 */
	public VirtualMachine clone(PhysicalMachine pm)
	{
		//set the pm to the cloned one
		VirtualMachine vm = new VirtualMachine(this.id, this.capacity, pm);
		vm.vmType = this.vmType;		
		vm.reserved = this.reserved;	
		
		//this will be set temporarily (to be able to use the vm.equals) and updated to the cloned request in vmProtection.clone()
		vm.request = this.request;
		vm.backupVm = this.backupVm;
	...
	public boolean equals(VirtualMachine vm) 
	{
		...
		//checking on request
		if ( vm.request!=null && this.request!=null)
		{
	...
	/**
	 * This function returns a string of Vm info
	 */
	public String toString()
	{
		String vmInfo =" ";
		vmInfo+="VM hosted on server "+this.pm.id+" ";
		vmInfo+=this.request!=null ?  " reserved for request "+this.request.id:"";
		vmInfo+=this.backupVm!=null? " its backup Vm is hosted on server "+this.backupVm.pm.id:"";
		vmInfo+="\n";
		return vmInfo;
