...
public class VirtualMachinesPlacement{
	...
	public VirtualMachinesPlacement(FatTreeNetwork treeNetwork)
	{
	...
	/**
	 * This function return fault domains represented by subtrees got from a certain switch level
	 * 
	 * @param faultDomainLevel switch level that represents the fault domain
	 * 
	 * @return array of subtrees representing fault domains at a certain level
	 */
	public SubTree [] getFaultDomains (int faultDomainLevel)
	{
		Switch [] faultDomainRoots;
		SubTree subTree;
		SubTree [] faultDomains;
		
		//get the switches at the specified level, they represent all the sub trees/fault domains
		faultDomainRoots = this.treeNetwork.getSwitchSetPerTreeLevel(faultDomainLevel);
		
		//create a subtree array that will contains all the subtrees representing the fault domains
		faultDomains = new SubTree [faultDomainRoots.length];
		
		//loop over the switches/faultDomainRoots
		for (int i = 0; i< faultDomains.length; i++)
		{	
			subTree = new SubTree(faultDomainRoots[i]) ;	
			
			//build the subTree
			subTree = this.treeNetwork.buildSubTree(subTree, subTree.rootNode);
			
			//add the subTree to the faultDomains array
			faultDomains[i] = subTree;			
		}
	...
	/**
	 * This function will place the request passed as parameter based on a WCS approach.
	 * That is; embedding one VM at a time in each fault domain defined by a switch (TOR switch).
	 * If a VM is placed in each fault domain, we keep on repeating to embed on VM per domain till all VMs are allocated.
	 * A random server from each domain is chosen to host a VM 
	 * 
	 * @param request
	 * @param faultDomainLevel level of switches to consider as fault domain (usually set to 1 for TOR switch
	 * @return array list of physical machines id and the number of VMs allocated for the request
	 */
	public ArrayList <int []> WCSVmPlacementForSingleRequest ( Request request, int faultDomainLevel)
	{
		int remainingVms = request.N;
		SubTree [] faultDomains;
		PhysicalMachine pm;
		int vmsToAllocateOnPm =1;
		Random rand = new Random();
		int randPmId =0;
		int [] allocation = null;
		int count =0;
		boolean allocatedVmsUpdated = false;
		int hostedVms =0;
		int bandwidthNeeded = 0;
		Link l;
		
		rand.setSeed(12);//to get the same sequence of random numbers at each run
		...
		//create a subtree that has the level 3 switch as rootNode (the whole network)
		...
		if (network.getAvailableVms() < request.N)
		{
			 return null;
		}
		
		//get all fault domains (sub tree) related to a certain switch level
		faultDomains = this.getFaultDomains (faultDomainLevel);
		
		//keep looping until N VMs are allocated
		while(remainingVms>0)
		{
			for (int i=0; i<faultDomains.length;i++)
			{
				//this condition is needed to prevent allocation of addition primary VMs, in case remainingVms become =0 before checking the while loop (still for loop condition true)
				if (remainingVms ==0)
				{
					break;
				}
				//generate random nb between 0 and the nb of servers in the subtree (exclusive)
				randPmId = rand.nextInt(faultDomains[i].physicalMachines.size());
				
				//get the pm at the specified random index of the specified fault domain
				pm = faultDomains[i].physicalMachines.get(randPmId);
				...
				//allocate one VM at a time in each fault domain. Make sure the allocation is possible
				pm.reserveVM(vmsToAllocateOnPm, request, VirtualMachine.Type.PRIMARY);					
				remainingVms -=vmsToAllocateOnPm;			
				
				//add the allocation to physicalVMAllocation array list; this is important for the first iteration because the code will not get into the for loop
				if (count ==0)
				{	
					allocation = new int[2];
					allocation[0] = pm.id;
					allocation[1] = vmsToAllocateOnPm;
				...
				for (int j=0; j<physicalVMAllocation.size(); j++)
				{	
					//if there is already VM assigned to this pm for this request update the number of VMs allocated
					if (physicalVMAllocation.get(j)[0] == pm.id)
					{
						physicalVMAllocation.get(j)[1]+=vmsToAllocateOnPm;
						allocatedVmsUpdated = true;
						break;
					}				
				}	
				
				//add new allocation if no VMs were already allocated on this server(pm)
				if(!allocatedVmsUpdated)
				{
					allocation = new int[2];
					allocation[0] = pm.id;
					allocation[1] = vmsToAllocateOnPm;
				...
				//reset the allocatedVmsUpdated
				allocatedVmsUpdated = false;
				count++;
			}
		}
		
		//allocate bandwidth on level 1 links
		for (int i=0; i<physicalVMAllocation.size(); i++)
		{
			allocation = physicalVMAllocation.get(i);
			pm = treeNetwork.physicalMachinesSet[allocation[0]];
		
			//search for the link having the physical machine as source node
			l = network.searchLink(pm);
			
			//get hosted Vms on pm
			hostedVms = pm.getHostedVms(request);
	
			//reserve bandwidth
			bandwidthNeeded =  hostedVms < (request.N-hostedVms) ? hostedVms*request.B :  (request.N-hostedVms)*request.B;
			
			if (!l.reserveBandwidth(bandwidthNeeded,request.id,Link.BandwidthType.PRIMARY))
			{
				treeNetwork.releaseAllocatedRequest(request, Type.PRIMARY);
				return null;
			}
		...
		if (!this.allocateUpperLevelBandwidth (network, request, physicalVMAllocation, true))
		{
			treeNetwork.releaseAllocatedRequest(request, Type.PRIMARY);
			return null;
		}
		
		//update the subtree and admitted attribute and reservedBandwidth attribute of the request 
		request.setSubTree(network);
	...
	/**
	 * This function provides a WCS embedding for all the requests
	 ...
	 * 
	 */	
	public void  WCSForMultipleRequests ( ArrayList<Request> requests,int faultDomainLevel, SubTree[] switchFailures, PhysicalMachine[]pmfailures) throws IloException, IOException
	{
		...
		for (int j=0; j<requests.size(); j++)
		{
			...
			request = requests.get(j);
			 ...
			 if (request.processType == Request.Type.ARRIVAL)
			 {
				...
				//allocate requests based on WCS
				physicalMachinesAllocation = this.WCSVmPlacementForSingleRequest(request, faultDomainLevel);
				...
				if (physicalMachinesAllocation == null|| physicalMachinesAllocation.size() == 0)
				{
					...
					this.printAllocationForRequest(request, physicalMachinesAllocation);					
					continue;
				}			
	...
	/**
	 * This function do the primary VMs placement of a request by randomly choosing a server and 
	 * the number of VMs to place on it. 
	 * 1-The server may be selected multiple times
	 * 2-if the number of VMs to place on the server is greater than the server capacity, we only place Vms = server capacity
	 * 3-The function performs bandwidth allocation and release the request from the network if it was not admitted	 *  
	 * If no bandwidth was available for the embedding the request is rejected
	 * 
	 * @param request
	 * @return array list of physical machines id and the number of VMs allocated for the request
	 */
	public ArrayList <int []> randomVmsPlacement ( Request request)
	{
		//array list of physical machine with VM allocated on them for each request
		ArrayList <int []> physicalVMAllocation = new ArrayList <int []>();		
		...
		while (remainingVms !=0)
		{
			...
			vmsToAllocateOnPm = rand.nextInt((remainingVms -1) + 1) + 1;
			
			vmsToAllocateOnPm =	vmsToAllocateOnPm > pm.getAvailableVM() ? pm.getAvailableVM()  : vmsToAllocateOnPm ;				
			...
			{	
				//if there is already VM assigned to this pm for this request update the number of VMs allocated
				if (physicalVMAllocation.get(i)[0] == pm.id)
				{
					physicalVMAllocation.get(i)[1]+=vmsToAllocateOnPm;
					allocatedVmsUpdated = true;
					break;
				}				
			}	
	...
	public ArrayList <int []> hoseVMPlacementAlgorithmSingleRequest ( Request request)
	{
		...
		{
			//this means that the request was not admitted 
			request.rejectionReason = RejectionReason.PRIMARY_EMBEDDING;
			return null;
		}
	...
	{	
		...
		//allocate bandwidth on the upper levels link (2,3)
		if (!this.allocateUpperLevelBandwidth (subTree, request, physicalVMAllocation, true))
		{
			request.rejectionReason = RejectionReason.PRIMARY_BANDWIDTH;
		}
	...
	public static void main (String [] args) throws IloException, IOException
	{
		...
		ArrayList<Request> requests = new ArrayList<Request>();
		//ArrayList <ArrayList <int []>> requestsAllocation= new ArrayList <ArrayList <int []>>();
		ArrayList <int []> requestsAllocation= new ArrayList <int []>();
		ArrayList <ArrayList <int []>> requestsAll= new ArrayList <ArrayList <int []>>();
		
		//int [] bandwidthRange = {300,600};
		int hoseRevenue = 0;
		double cplexSolution = 0;
		Request r= new Request (0,5, 100);
		requests.add(r);
		Request r1= new Request (1,8, 200);
		requests.add(r1);
		...
		requestsAllocation = vmPlacement.randomVmsPlacement(r);
		//requestsAllocation = vmPlacement.WCSVmPlacementForSingleRequest(r, 1);
		// vmPlacement.WCSForMultipleRequests(requests, 1);
		//requestsAll.add(requestsAllocation);
		...
		vmPlacement.printRequestsAllocation (requests, requestsAll);
		Link[] links;
		links = treeNetwork.getLinks();
