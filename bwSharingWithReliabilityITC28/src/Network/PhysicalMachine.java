...
public class PhysicalMachine extends Node {
	...
	/**
	 * This function clones a physical machine
	 * 
	 * @return cloned physical server
	 */
	public PhysicalMachine clone ()
	{
		PhysicalMachine pm = new PhysicalMachine(this.id, this.nbOfVM);
		
		//set to null because it was initialized by the constructor
		pm.virtualMachines = new VirtualMachine [this.nbOfVM];
		
		for(int i= 0; i<this.virtualMachines.length; i++)
		{
			pm.virtualMachines[i] = this.virtualMachines[i].clone(pm);
		}
	...
	/**
	 * This function checks if 2 physical servers are equal
	 * @param pm physical machine to check
	 * 
	 * @return boolean
	 */
	public boolean equals (PhysicalMachine pm)
	{
		if (this.id != pm.id)
		{
			return false;
	...
	public String toString()
	{
		...
		machineInformation +="VMs-----------------\n";
		for (int i=0; i<this.virtualMachines.length; i++)
		{
			machineInformation +=this.virtualMachines[i].toString();
		}
