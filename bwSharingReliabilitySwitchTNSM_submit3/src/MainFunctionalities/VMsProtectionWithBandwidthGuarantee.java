...
public class VMsProtectionWithBandwidthGuarantee {
	 ...
	 * It is required by reviewer in 2nd review for the journal
	 * We embed N primary VM based on the hose and embed the min number of backup vm on a server not 
	 * hosting any primary VM. If no server can host all the backup we reject the request.
	 * No need to run the PPD model for the correspondence because all the backup will be on the same server
	 * so bw can be allocated based on the hose model
	 *
	...
	public void backupEmbeddingNaiveNL ( int faultDomainLevel, SubTree[] switchFailures, PhysicalMachine[]pmfailures) throws IloException, IOException
	{
		...
		for (int j=0; j<this.requests.size(); j++)
		{
			...
			request = this.requests.get(j);
			 ...
			 if (request.processType == Request.Type.ARRIVAL)
			 {
				...
				//protect the request by embedding all backups on one server + reserving backup bandwidth
				//this will directly release the request from the network and set it as not admitted if it can not be protected
				this.naiveRandomRequestProtection(request, physicalMachinesAllocation);
	...
	/**
	 * This function embed the backup VMs on a server not hosting any primary VMs and reserves the backup
	 * bandwidth needed while applying bw reuse.
	 *  The request is rejected if no server can accommodate all the backup VMs all not enough bw.
	 * No need to specify the primary to backup correspondence because all backups are hosted on the same server
	 * @param request
	 * @param physicalMachinesAllocation allocation of the primary VMs of the request
	 * @return true if protection is possible
	 */
	public boolean naiveRandomRequestProtection(Request request, ArrayList<int[]> physicalMachinesAllocation)
	{
		int backupNeeded = request.getBackupNeeded();
		ArrayList<PhysicalMachine> serversNotHostingPrimaryVM;
		...
		//get servers not hosting any 
		serversNotHostingPrimaryVM = network.getNonHostingServers(request);
		//order servers in descending order of their available VMs
		serversNotHostingPrimaryVM = network.orderMachinesByAvailableVms(serversNotHostingPrimaryVM);
		
		//check if the server with highest availability has enough VM to host all backups. If not reject the request
		if (serversNotHostingPrimaryVM.get(0).getAvailableVM()<backupNeeded)
		{
			 request.admitted = false;
		...
		for(int i=0; i<serversNotHostingPrimaryVM.size(); i++)
		{
			//start with the server with the biggest amount of available VM to host the backup
			pm = serversNotHostingPrimaryVM.get(i);
			
			//if we reach a server with not enough VMs this means that not enough bw in the network to accomodate the request break
			if (pm.getAvailableVM()<backupNeeded)
			{
				 request.admitted = false;
			...
			//check if there is enough bandwidth to host the backup on the upper level links
			if (!this.allocateBackupBandwidth(network, request, physicalMachinesAllocation, pm, false))
			{
				continue;
			}
		
			//allocate backup bandwidth and VMs		
			pm.reserveVM(backupNeeded, request, VirtualMachine.Type.BACKUP);
			
			this.allocateBackupBandwidth(network, request, physicalMachinesAllocation, pm, true);
		...
		//if we reach here than not enough bw to protect the request --release
		request.admitted = false;
		request.rejectionReason = Request.RejectionReason.BACKUP_MAPPING_BANDWIDTH;
	 ...
	 * @param backupServer the server hosting the backup VM
	 * @param reserve true if we want to reserve the bandwidth
	 * @return true if the bandwidth can be reserved
	 */
	public boolean allocateBackupBandwidth (SubTree subTree, Request request, ArrayList <int[]> physicalVMAllocation, PhysicalMachine backupServer, boolean reserve)
	{
		ArrayList<PhysicalMachine> hostingServers = null; 
		ArrayList<PhysicalMachine> hostingServersUpdated = new ArrayList<PhysicalMachine>(); 
		PhysicalMachine failedServer = null;
		int backupbw =0;
		int vmOnServer =0;
		int vmsUnderSwitch=0;
		Link l= null;
		int nbOfLinks = this.treeNetwork.pmToTorLinkSet.length+this.treeNetwork.torToAggregateLinkSet.length+this.treeNetwork.aggregateToCoreLinkset.length;
		ArrayList<Integer>linkBackupBw = new ArrayList<Integer>();//index is the link continious id
		
		//initialize the array and prevent exception
		for(int i=0; i<nbOfLinks;i++)
		{
			linkBackupBw.add(i, 0);
		}
		
		
		//get servers hosting primary vms
		hostingServers = subTree.getHostingServers(request);
		
		//copy the hosting servers and include the backup server
		hostingServersUpdated.addAll(hostingServers);
		hostingServersUpdated.add(backupServer);
		
		//consider the failure of each hosting server
		for (int i=0; i<hostingServers.size(); i++)
		{
			failedServer = hostingServers.get(i);
						
			//start by calculating the bandwidth on the links connecting the servers hosting the VMs
			for(int j=0; j<hostingServersUpdated.size(); j++)
			{
				//no backup bandwidth on the link connecting the failed server
				if (j==i)
				{
					continue;
				}
				
				if(hostingServersUpdated.get(j).equals(backupServer))
				{
					//consider that active backup are equal failed primary vms
					vmOnServer = failedServer.getHostedVms(request);
				}
				else
				{
					vmOnServer = hostingServersUpdated.get(j).getHostedVms(request);
				}
				
				//since the number of activated backup upon the failure of pm = nb of failed vm we always have N VMs in the network. This why the number
				//VMs hosted on the other servers = request.N-vmOnServer
				backupbw = vmOnServer*request.B<(request.N-vmOnServer)*request.B ? 
						vmOnServer*request.B:(request.N-vmOnServer)*request.B;
				
				l = subTree.searchLink(hostingServersUpdated.get(j));
				
				//reuse of primary bandwidth
				if (l.getBandwidthAllocatedForRequest(request.id)!=null)
				{
					backupbw = backupbw >= l.getBandwidthAllocatedForRequest(request.id)[1] ? backupbw-l.getBandwidthAllocatedForRequest(request.id)[1] :backupbw;
					
				}
				
				//link does not have enough bw we need to try a new backup embedding
				if(l.bandwidth<backupbw)
				{
					return  false;
				}
				
				if (linkBackupBw.get(l.continuousId)<backupbw)
				{
					//if the backup bw on a link upon a certain failure is less than the current one update it to hold the max backup bw
					linkBackupBw.set(l.continuousId,backupbw);
				}
			}
			
			//calculate backup bw for upper level links
			for (int k=0; k<subTree.switches.size(); k++)
			{ 
				//get link having the switch as source node, this ensures not getting the link related to Pm
				l = subTree.searchLink(subTree.switches.get(k));
				...
				//get the number of VMs allocated under the switch this will include the backup VM since they are included in physicalVMAllocation
				vmsUnderSwitch = subTree.getActiveVMUponFailure(physicalVMAllocation, subTree.switches.get(k),failedServer, backupServer);
				backupbw = vmsUnderSwitch*request.B<(request.N-vmsUnderSwitch)*request.B ? 
						vmsUnderSwitch*request.B:(request.N-vmsUnderSwitch)*request.B;
		...
		if (reserve)
		{
			for(int i=0; i<linkBackupBw.size(); i++)
			{
				l = this.treeNetwork.getLinkPerContiniousId(i);
				if (linkBackupBw.get(i) == null)
				{
					//no reservation on the link
					continue;
				}
				l.reserveBandwidth((int)linkBackupBw.get(i),request.id,Link.BandwidthType.BACKUP);
	...
	public void printRequestsInformation (int load, int setNb, ArrayList<int[]> backupToVmMappingModelTestBw ,	ArrayList<int[]> backupEmbeddingBaselineBw, ArrayList<int[]> WCSBw) throws IOException
	{
		...
		String naiveNLRequestsBw = "TestResults/naiveNLRequestsBw.txt";
		FileManipulation naiveNLBwFile  = new FileManipulation(naiveNLRequestsBw);
		...
		*/
		...
		String naiveNLRequestsBackupVMs = "TestResults/naiveNLRequestsBackupVMs.txt";
		FileManipulation naiveNLBackupFile  = new FileManipulation(naiveNLRequestsBackupVMs);
	...
	/*	backupToVmMappingModelTestBwFile.writeInFile("\n\n====Reserved bandwidth for admitted requests  by backupToVmMappingModelTest () ====\n");
		...
		naiveNLBwFile.writeInFile("\n\n====Reserved bandwidth for admitted requests  by naiveNL () ====\n");
		naiveNLBwFile.writeInFile("====LOAD: "+load+" SET NB: "+setNb+"====\n");
		...
		naiveNLBackupFile.writeInFile("\n\n====Reserved Backup VMS  for admitted requests  by naiveNL () ====\n");
		naiveNLBackupFile.writeInFile("====LOAD: "+load+" SET NB: "+setNb+"====\n");
		...
		for (int i=0; i< requestsBwComparision.size(); i++)
		{
			...
			WCSBwFile.writeInFile(requestsBwComparision.get(i)[1]+"\n");
			naiveNLBwFile.writeInFile(requestsBwComparision.get(i)[2]+"\n");
			...
			WCSBackupFile.writeInFile(requestsBwComparision.get(i)[3]+"\n");
			naiveNLBackupFile.writeInFile(requestsBwComparision.get(i)[4]+"\n");
		}
	}
	...
	public ArrayList<int[]> automatedTesting (String algorithm, ArrayList<int[]> randomVmsBw, Poisson poissonDistribution, int setNb, double alpha ) throws IOException, IloException
	{
		...
		else if (algorithm.equals("naiveNL"))
		{
			vmsProtection.backupEmbeddingNaiveNL( faultLevel, switchFaults, pmFaults);
		}
		else
		{
	...
	{
		String admittedRequestsIdFile = "TestResults/admittedRequestsId_naiveOnly.txt";
		FileManipulation admittedRequestsFile  = new FileManipulation(admittedRequestsIdFile);
		String naiveNLRequestsBwFile = "TestResults/naiveNLRequestsBw.txt";
		FileManipulation  naiveNLBwFile  = new FileManipulation(naiveNLRequestsBwFile);
		String naiveNLBackupVMFile = "TestResults/naiveNLBackupVM.txt";
		FileManipulation  naiveNLBackupVMsFile  = new FileManipulation(naiveNLBackupVMFile);
	...
	//	statusFile.writeInFile("\n=============================Network : FatTreeNetwork(256,6,2,2,64,1000,10000,10000);=============================\n");
		...
		for (int i = 0; i<arrivalRates.length; i++)
		{
			...
			for (int j=0; j<setsNb; j++)
			{
				...
				statusFile.writeInFile("=Starting test for backupEmbeddingNaiveNL()=\n");
				
				naiveNlBw = vmsProtection.automatedTesting("naiveNL",randomVmsBw,poissonDistribution, j,alpha);
				...
				admittedRequestsFile.writeInFile("==Load "+arrivalRates[i]/departureRate+" set "+j+"===\n");
				naiveNLBwFile.writeInFile("==Load "+arrivalRates[i]/departureRate+" set "+j+"===\n");
				naiveNLBackupVMsFile.writeInFile("==Load "+arrivalRates[i]/departureRate+" set "+j+"===\n");
				for (int k=0; k< naiveNlBw.size(); k++)
				{
					//skip requests that were not admitted by both algorithms
					if (naiveNlBw.get(k)[1] == -1 )
					{
						continue;
					}
					
					admittedRequestsFile.writeInFile(naiveNlBw.get(k)[0]+"\n");
					naiveNLBwFile.writeInFile(naiveNlBw.get(k)[1]+"\n");
					naiveNLBackupVMsFile.writeInFile(naiveNlBw.get(k)[2]+"\n");
				}
