...
public class FatTreeNetwork {
	...
	/**
	 * This function returns the number of active VMs (primary+backup) for a request upon the failure of a subtree (subtree can not be a server)
	 * @param request request to which we want the nb of active vms
	 * @param subtree failed subtree
	 * @return nb of active vms
	 */
	public Double getActiveVMs(Request request, SubTree subtree)
	{
		Double activeVms =0.0;//needed to calculate fault tolerance
		VirtualMachine vm;
		ArrayList <Integer> backupVmsId = new ArrayList<Integer>();
		for(int i=0; i<request.virtualMachinesSet.size(); i++)
		{
			vm = request.virtualMachinesSet.get(i);
			...
			//if the vm is primary and belongs to a failed server (in the failed subtree) don't count it and count its backup vm
			if (subtree.belongToSwitchSubtree((Switch)subtree.rootNode, vm.pm.id,false ))
			{	
				//make sure that the backup vm is not in the failed subtree(safe check), count only backup backing up the failed servers
				if ( vm.backupVm!=null && !subtree.belongToSwitchSubtree((Switch)subtree.rootNode, vm.backupVm.pm.id, false))
				{
					//make sure that a backup VM is counted once if it is backing up more than one failed primary VMs
					if (!backupVmsId.contains(vm.backupVm.id))
					{
						backupVmsId.add(vm.backupVm.id);
						activeVms++;
					}
			...
			//the primary does not belong to the failed subtree so count it
			activeVms++;
		}
	...
	/**
	 * This function returns the number of active VMs (primary+backup backing up failed VMS) for a request upon the failure of a server 
	 * @param request request to which we want the nb of active vms
	 * @param PhysicalMachine failed server
	 * @return nb of active vms
	 */
	public Double getActiveVMs(Request request, PhysicalMachine pm)
	{
		Double activeVms =0.0;//needed to calculate fault tolerance
		VirtualMachine vm;
		ArrayList <Integer> backupVmsId = new ArrayList<Integer>();
		for(int i=0; i<request.virtualMachinesSet.size(); i++)
		{
			...
			if (vm.pm.id == pm.id)
			{	
				//make sure that the backup vm is not in the failed server, count only backup backing up the failed servers
				if ( vm.backupVm!=null && vm.backupVm.pm.id!=pm.id)
				{
					//make sure that a backup VM is counted once if it is backing up more than one failed primary VMs
					if (!backupVmsId.contains(vm.backupVm.id))
					{
