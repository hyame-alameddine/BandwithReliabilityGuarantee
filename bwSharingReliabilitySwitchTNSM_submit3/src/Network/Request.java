...
public class Request {
	...
	//set of primary and backups virtual machines of the request. This is set when reserving Vms and will not be kept set after releasing the request
	...
	//array that will hold fault tolerance of request upon pm/switch failure. this is kept after the request leaves the network
	public ArrayList<Double> pmFaultTolerance;
	public ArrayList<Double> switchFaultTolerance;
	...
	public Request (int id, int N, int B)
	{
		...
		this.pmFaultTolerance = new ArrayList<Double>();
		this.switchFaultTolerance = new ArrayList<Double>();
	...
	public Request clone()
	{
		...
		r.pmFaultTolerance = new ArrayList<Double>();
		r.switchFaultTolerance = new ArrayList<Double>();
		return r;
	...
	public Request cloneAll( FatTreeNetwork clonedNetwork )
	{
		...
		for (int i =0; i<this.switchFaultTolerance.size(); i++)
		{	
			d = this.switchFaultTolerance.get(i);
			r.switchFaultTolerance.add(d);
			d=null;
		}
		
		for (int i =0; i<this.pmFaultTolerance.size(); i++)
		{	
			d = this.pmFaultTolerance.get(i);
			r.pmFaultTolerance.add(d);
			d=null;
		}
	...
	/**
	 * Calculate the fault tolerance of the request upon failure of each server specified in faults
	 * FT = activeVMs after failure/N
	 * @param faults physical machines faults
	 */
	public void setPmFaultTolerane(PhysicalMachine[]faults)
	{
		//get active Vms (primary and backup backing up the failed VM) upon the failure of the specified server
		Double activeVms = 0.0;
		Double faultTolerance;
		for (int i=0; i<faults.length; i++)
		{
			activeVms = this.subtree.fatTreeNetwork.getActiveVMs(this, faults[i]);
			faultTolerance = activeVms/this.N;
			this.pmFaultTolerance.add(faultTolerance);
			faultTolerance =0.0;
			activeVms =0.0;
		}
	}
	...
	/**
	 * Calculate the fault tolerance of the request upon failure of each switch specified in faults
	 * FT = activeVMs after failure/N
	 * @param faults physical machines faults
	 */
	public void setSwitchFaultTolerane(SubTree[]faults)
	{
		//get active Vms (primary and backup backing up the failed VM) upon the failure of the specified switch/subtree
		Double activeVms = 0.0;
		Double faultTolerance;
		for (int i=0; i<faults.length; i++)
		{
			...
			faultTolerance =activeVms/this.N;
			this.switchFaultTolerance.add(faultTolerance);
			faultTolerance =0.0;
			activeVms =0.0;
		}
	}
