...
public class SubTree {
	...
	/**
	 * This function returns the number of VMs allocated under the specified switch upon the failure of a server
	 * while considering that a backup server hosts an equal number of failed VMs and considered as part of the VMs
	 * hosted under the switch if the backup server is under the switch
	 * 
	 * @param physicalVMAllocation array list of <pmId, VmAllocated> for the request
	 * @param switch s, switch  which belongs to this subtree that we want to get the VM allocated under it 
	 * @param failed  server which we do not want to consider the hosted vms
	 * @param backup server server hosting the backup vms
	 * @return number of VMs (primary/backup) active inthe pm under the switch for the specified allocation
	 */
	public int getActiveVMUponFailure (ArrayList <int[]> physicalVMAllocation, Switch s, PhysicalMachine failedServer, PhysicalMachine backupServer)
	{
		int allocatedVMs = 0;
		int [] allocation ;
		int failedVms =0;
		...
		for (int i=0; i<physicalVMAllocation.size(); i++)
		{
			...
			if (allocation[0] == failedServer.id)
			{
				failedVms = allocation[1];
				continue;
			}
		...
		if ( this.belongToSwitchSubtree (s,backupServer.id, false))
		{
			allocatedVMs+= failedVms;
		}
